...
def main() -> None:
    ...
    st.set_page_config(page_title="Skracacz URL", page_icon="🔗", layout="wide")
        ...
        with st.form("url_form", clear_on_submit=True):
            ...
            if submitted:
                if not validate_url(original_url):
                    st.error(
                        " Nieprawidłowy URL! URL musi zaczynać się od http:// lub https://"
                    )
                else:
                    try:
    ...
    if not urls:
        ...
        if search_term:
            urls = [
                url
                for url in urls
                if search_term.lower() in url.original_url.lower()
                or search_term.lower() in url.short_code.lower()
            ]

        for url in urls:
            with st.expander(
                f"🔗 {url.short_code} → {url.original_url[:50]}..."
                if len(url.original_url) > 50
                else f"🔗 {url.short_code} → {url.original_url}"
            ):
                ...
                with col_a:
                    ...
                    st.write(
                        f"**Utworzono:** {url.created_at.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                    st.write(f"**Liczba kliknięć:** {url.click_count} 👆")

                with col_b:
                    if st.button("Usuń", key=f"delete_{url.id}"):
